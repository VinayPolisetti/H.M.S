...
const DropContent = ({ date: initialDate, time: initialTime, onClose }) => {
  ...
  const close = () => {
    ...
    let endmin  =parseInt(parsedTime[1], 10);
    console.log(endmin); 
    //rejoin into string
            ...
            onSubmit={({ value }) => {
              ...
              fetch("http://localhost:3001/userInSession")
                ...
                .then(res => {
                  ...
                  console.log(theDate);
                  fetch("http://localhost:3001/checkIfApptExists?email=" + email_in_use + "&startTime=" + theTime + "&endTime=" + endTime +"&date=" + theDate + "&docEmail=" + theDoc)
                    .then(res => res.json())
                    .then(res => {
                      if ((res.data[0])) {
